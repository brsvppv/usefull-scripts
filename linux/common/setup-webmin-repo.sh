...
Repository types:
  --stable                   Stable repo
  --prerelease               Prerelease repo
  --unstable                 Unstable repo

Repository configuration and metadata:
  --host=<host>              Repository host
  --key=<key>                Signing key file
  --key-suffix=<suffix>      Key suffix for file naming
  --auth-user=<user>         Repo auth user
  --auth-pass=<pass>         Repo auth password
  --name=<name>              Base name for repository (default: webmin)
  --description=<desc>       Repository description
  --component=<comp>         Component (default: main)
  --section=<sec>            Section (default: contrib)
  --dist=<dist>              Distribution (default: stable)

Post-installation options:
  --check-binary=<path>      Binary to validate installation
  --install-message=<msg>    Message shown if binary not found
  --install-packages=<pkgs>  Suggested packages to install

Examples:
  $(basename "$0") --stable --host=download.webmin.com
  $(basename "$0") --unstable --host=download.webmin.dev
EOF
  exit 1
}
